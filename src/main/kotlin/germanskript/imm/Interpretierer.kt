...
import germanskript.*
import germanskript.imm.intern.*
import java.io.File
import java.util.*
...
class Interpretierer(startDatei: File): PipelineKomponente(startDatei), IInterpretierer {
  ...
  class AufrufStapel {
    ...
    private fun aufrufStapelElementToString(element: AufrufStapelElement): String {
      ...
      if (element.objekt is germanskript.intern.Objekt) {
  ...
  private fun interpretiereSolangeSchleife(schleife: IMM_AST.Satz.SolangeSchleife): Objekt {
    while (!flags.contains(Flag.SCHLEIFE_ABBRECHEN) &&
        ...
        (interpretiereAusdruck(schleife.bedingungsTerm.bedingung) as germanskript.imm.intern.Boolean).boolean
    ) {
  ...
  private fun interpretiereObjektInstanziierung(instanziierung: IMM_AST.Satz.Ausdruck.ObjektInstanziierung): Objekt {
    ...
    return when(instanziierung.klasse.name) {
      ...
      "HashMap" -> germanskript.imm.intern.HashMap(instanziierung.typ)
      "Datei" -> Datei()
      else ->
  ...
  private fun interpretiereBedingungsTerm(term: IMM_AST.Satz.BedingungsTerm): Objekt? {
    return if (!sollteStackAufrollen() && (interpretiereAusdruck(term.bedingung) as germanskript.imm.intern.Boolean).boolean) {
  ...
  private fun interpretiereTypÜberprüfung(typÜberprüfung: IMM_AST.Satz.Ausdruck.TypÜberprüfung): Objekt {
    ...
    val istTyp = typPrüfer.typIstTyp(typ, typÜberprüfung.typ.typ!!)
    return Boolean(istTyp)
  }

  private fun interpretiereLogischesUnd(logischesUnd: IMM_AST.Satz.Ausdruck.LogischesUnd): Objekt {
      return Boolean(
          (interpretiereAusdruck(logischesUnd.links) as germanskript.imm.intern.Boolean).boolean &&
              (interpretiereAusdruck(logischesUnd.rechts) as germanskript.imm.intern.Boolean).boolean
      )
  }

  private fun interpretiereLogischesOder(logischesOder: IMM_AST.Satz.Ausdruck.LogischesOder): Objekt {
    return Boolean(
        (interpretiereAusdruck(logischesOder.links) as germanskript.imm.intern.Boolean).boolean ||
            (interpretiereAusdruck(logischesOder.rechts) as germanskript.imm.intern.Boolean).boolean
    )
  }

  private fun interpretiereLogischesNicht(logischesNicht: IMM_AST.Satz.Ausdruck.LogischesNicht): Objekt {
    return Boolean(!(interpretiereAusdruck(logischesNicht.ausdruck) as germanskript.imm.intern.Boolean).boolean)
  }
